...
class webhookModel extends model
{
    ...
    public function create($webhook)
    {
        ...
        {
            $webhook = $this->webhookTao->getRocketChatSecret($webhook);
        }

        if(dao::isError()) return false;

        $this->dao->insert(TABLE_WEBHOOK)->data($webhook, 'agentId,appKey,appSecret,wechatCorpId,wechatCorpSecret,wechatAgentId,feishuAppId,feishuAppSecret,rocketChannel,rocketXAuthToken,rocketXUserId')
    ...
    public function update($id, $webhook)
    {
        ...
        $this->dao->update(TABLE_WEBHOOK)->data($webhook, 'agentId,appKey,appSecret,wechatCorpId,wechatCorpSecret,wechatAgentId,feishuAppId,feishuAppSecret,rocketChannel,rocketXAuthToken,rocketXUserId')
    ...
    public function fetchHook($webhook, $sendData, $actionID = 0, $appendUser = '')
    {
        ...
        if($webhook->type == 'rocketchannel') {
            $logPath = dirname(dirname(dirname(__FILE__))) . '/tmp/log/webhook.log';
            //先发送给已绑定的用户
            $this->sendToUser($webhook, $sendData, $actionID, $appendUser);

            $ch = $this->curlInit($url, $header);
        
            $tmpdata = json_decode($sendData);

            //file_put_contents($logPath, date('Y-m-d H:i:s') . " | before send | channel: " . $tmpdata->channel . "\n", FILE_APPEND);
            ...
            if(strlen($tmpdata->channel) > 0) {
                ...
                foreach($channels as $channel) {
                    ...
                    file_put_contents($logPath, date('Y-m-d H:i:s') . " | channel: " . $tmpdata->channel . " | text: " . $tmpdata->text . "\n", FILE_APPEND);

                    $result   = curl_exec($ch);
                    $httpCode = curl_getinfo($ch, CURLINFO_HTTP_CODE);
                    $error    = curl_error($ch);
                    curl_close($ch);
                    file_put_contents($logPath, date('Y-m-d H:i:s') . " | after send | channel: " . $tmpdata->channel . " | result: " . $result . "\n", FILE_APPEND);
            ...
            file_put_contents($logPath, date('Y-m-d H:i:s') . " | openIdList: " . $openIdList . "\n", FILE_APPEND);

            if(!empty($openIdList)) {
                $openIdArr = explode(',', $openIdList);
    ...
    public function sendToUser($webhook, $sendData, $actionID, $appendUser)
    {
        ...
        elseif($webhook->type == 'rocketchannel')
        {
            ...
            $rocketapi = new rocketchatapi($webhook->secret->appId, $webhook->secret->appSecret, $webhook->secret->apiUrl);
            $result  = $rocketapi->send($openIdList, $sendData, $webhook->url);
            return json_encode($result);
        }
    }
