...
class webhookTao extends webhookModel
{
     ...
     * @return object|false
     */
    protected function getDingdingSecret($webhook)
    {
     ...
     * @return object|false
     */
    protected function getWeixinSecret($webhook)
    {
        $webhook->secret = array();
     ...
     * @return object|false
     */
    protected function getFeishuSecret($webhook)
    {
        $webhook->secret = array();
    ...
    /**
     * 获取 RocketChat 的密钥。
     * Get the X-Auth of RocketChat.
     *
     * @param  object       $webhook
     * @access public
     * @return object|false
     */
    protected function getRocketChatSecret($webhook)
    {
        $webhook->secret = array();
        $webhook->secret['appId']     = $webhook->rocketXUserId;
        $webhook->secret['appSecret'] = $webhook->rocketXAuthToken;

        if(empty($webhook->rocketXUserId))    dao::$errors['rocketXUserId']    = sprintf($this->lang->error->notempty, $this->lang->webhook->rocketXUserId);
        if(empty($webhook->rocketXAuthToken)) dao::$errors['rocketXAuthToken'] = sprintf($this->lang->error->notempty, $this->lang->webhook->rocketXAuthToken);
        if(dao::isError()) return false;

        $webhook->params = $webhook->rocketChannel;
        $parts = explode('/hooks/', $webhook->url);
        $webhook->secret['apiUrl'] = $parts[0] . "/api/v1/";
     ...
     * @return string
     */
    protected function getActionText($data, $action, $object, $users)
    {
        $text = '';
